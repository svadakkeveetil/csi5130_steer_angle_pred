...
out_name = model_path.replace("checkpoints", "submissions").replace("hdf5", "csv")
...
filenames = glob.glob("{}phase2_test/center/*.jpg".format(image_path))
filenames = sorted(filenames)
#frame_ids = [f.replace(".jpg", "").replace("{}/round2/test/center/".format(data_path), "") for f in filenames]
frame_ids = [f.split('/')[-1].replace(".jpg", "") for f in filenames]
